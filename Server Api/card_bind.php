...
if(isset($_POST['pho'])&&isset($_POST['acs'])&&isset($_POST['num'])&&isset($_POST['pas'])){
	$pho=$_POST['pho'];
	$acs=$_POST['acs'];
	$num=$_POST['num'];
	$pas=$_POST['pas'];
	...
	if(mysql_num_rows(mysql_query("select * from user where pho='$pho' and acs='$acs'"))>0){
		...
		if(//卡卷有效性的判断
			mysql_num_rows(mysql_query("select * from card where id='$num' and psd='$pas' and status>3"))>0&&//账号密码正确//卡卷已激活
