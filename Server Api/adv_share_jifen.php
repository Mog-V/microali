...
if(isset($_POST['pho'])&&isset($_POST['acs'])&&isset($_POST['advid'])){
	...
	if(mysql_num_rows(mysql_query("select * from user where pho='$pho' and acs='$acs'"))>0){
            if(mysql_num_rows(mysql_query("select * from adv_share_ed where pho='$pho' and advid='$advid'"))==0){
                $num=rand(40,80);
                mysql_query("insert into adv_share_ed(advid,pho,num,time) values('$advid','$pho','$num','$time')");
                $return['status']=0;
                $return['val']=$num;
                mysql_query("COMMIT");
            }
